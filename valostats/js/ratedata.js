$(document).ready(function() {
    ...
    var myPolarChart; // Corrected variable name

    function getDataFromDB(ctype) {
        $.ajax({
            type: 'post',
            url: 'includes/firerate_data.php',
            data: { action: 'getData' },
            dataType: 'json',
            success: function(response) {
                ...
                populateList("#lowestRates", response.lowestRates);

                // Populate Top 5 Highest Prices
                populateList("#highestRates", response.highestRates);
            },
    ...
    function reloadChart(chartType, labels, datasets, backColor) {
        if (myPolarChart) {
            myPolarChart.destroy(); // Destroy previous chart
        }
    ...
    function populateList(selector, items) {
        ...
        items.forEach(item => {
            listElement.append(`<li class="list-group-item">${item.Name}: ${item.Fire_Rate}</li>`);
        });
    }

    getDataFromDB('polarArea');
});
